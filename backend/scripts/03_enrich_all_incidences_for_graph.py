...
def _parse_year(created_date: str) -> str:
    raw = (created_date or "").strip()
    if not raw:
        return "unknown"
...
def main() -> int:
    ...
    for row in rows:
        ...
        area_cluster_raw = _normalize_airtable_list(row.get("Area cluster", ""))
        area_cluster = area_cluster_raw
        ...
        enriched = {
            ...
            "Days to evaluate": row.get("Days to evaluate", ""),
            ...
            "Cost": row.get("Cost", ""),
            "Cost Responsibility": row.get("Cost Responsibility", ""),
    ...
    with OUTPUT_CSV.open("w", encoding="utf-8", newline="") as f:
        ...
        for item in enriched_records:
            out = {col: item.get(col, "") for col in csv_columns}
            out["has_incidence_docs"] = "1" if item["has_incidence_docs"] else "0"
            out["has_any_attachment"] = "1" if item["has_any_attachment"] else "0"
            out["missing_questions_json"] = json.dumps(item["missing_questions"], ensure_ascii=False)
            writer.writerow(out)
    ...
    category_dist = Counter(item["inferred_category"] for item in enriched_records)
    status_dist = Counter(item["Status"] or "unknown" for item in enriched_records)
    year_dist = Counter(_parse_year(item.get("Created date", "")) for item in enriched_records)
    records_with_description = sum(1 for item in enriched_records if _is_non_empty(item.get("clean_description", "")))
    records_with_property_context = sum(1 for item in enriched_records if _is_non_empty(item.get("property_context_id", "")) and not str(item.get("property_context_id", "")).startswith("unknown_property::"))
    records_with_provider_candidate = sum(1 for item in enriched_records if _is_non_empty(item.get("provider_candidate", "")))
    ...
    print(f"records with provider_candidate: {records_with_provider_candidate}")
